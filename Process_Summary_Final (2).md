...
```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#EAF3FF', 'primaryTextColor': '#000000', 'primaryBorderColor': '#6699CC', 'lineColor': '#333333', 'fontSize': '16px', 'edgeLabelBackground': '#FFFFFF' }}}%%
flowchart TD
    subgraph CLIENT["Client"]
        C1["Lead arrives via referral, LinkedIn, or website"]
        C2{"Reviews proposal"}
        C3["Signs agreement"]
        C4["Receives status updates"]
        C5["Receives and pays invoice"]
    end

    subgraph SALES["Managing Partner or Consultant"]
        S1["Runs discovery call"]
        S2{"Is it a fit"}
        S3["Authors proposal"]
        S4["Assigns work directly for high priority clients"]
    end

    subgraph PM["Project Manager"]
        M1["Creates Asana project and shared folders"]
        M2["Assigns tasks by availability and expertise"]
    end

    subgraph DELIVERY["Consultant, Delivery"]
        D1["Executes delivery work"]
        D2["Updates status weekly"]
        D3["Submits time entries"]
    end

    subgraph ACCT["Accounting"]
        A1["Generates invoice"]
    end

    C1 --> S1 --> S2
    S2 -- No --> DECLINE["Opportunity declined"]
    S2 -- Yes --> S3 --> C2
    C2 -- Requests revision --> S3
    C2 -- Accepts --> C3
    C3 --> M1 --> M2 --> D1
    S4 -.direct override.-> M2
    C3 -.occasionally, before setup complete.-> D1
    D1 --> D2 --> C4
    D1 --> D3 --> A1 --> C5

    style CLIENT fill:#FFFFFF,stroke:#B3D1F0,color:#000000
    style SALES fill:#FFFFFF,stroke:#B3D1F0,color:#000000
    style PM fill:#FFFFFF,stroke:#B3D1F0,color:#000000
    style DELIVERY fill:#FFFFFF,stroke:#B3D1F0,color:#000000
    style ACCT fill:#FFFFFF,stroke:#B3D1F0,color:#000000

    classDef stepNode fill:#EAF3FF,stroke:#6699CC,stroke-width:1.5px,color:#000000;
    classDef decisionNode fill:#FFE8CC,stroke:#E8A33D,stroke-width:1.5px,color:#000000;

    class C1,C3,C4,C5,S1,S3,S4,DECLINE,M1,M2,D1,D2,D3,A1 stepNode;
    class C2,S2 decisionNode;

    linkStyle 10 stroke:#E8A33D,stroke-width:1.5px;
    linkStyle 11 stroke:#E8A33D,stroke-width:1.5px;
```
